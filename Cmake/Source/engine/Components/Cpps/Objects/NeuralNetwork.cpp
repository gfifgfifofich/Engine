					...
					unsigned int* OutTexture, unsigned int packedInTexture,unsigned int packedWeightsTexture,unsigned int packedBiasesTexture)
{
	...
	glUniform1i(glGetUniformLocation(NNShader, "biases"), 0);

	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, packedInTexture);
	glUniform1i(glGetUniformLocation(NNShader, "data_in"), 2);

	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, packedWeightsTexture);
	glUniform1i(glGetUniformLocation(NNShader, "weights"), 1);

	glBindVertexArray(ScreenVAO);
	glUniform1i(glGetUniformLocation(NNShader, "inSize"), inSize);
	glUniform1i(glGetUniformLocation(NNShader, "outSize"), outSize);
	glUniform1i(glGetUniformLocation(NNShader, "InstanceCount"), InstanceCount);
	glUniform1i(glGetUniformLocation(NNShader, "Func"), Func);
